...
Ext.define('Ext.ux.exporter.Button', {
  ...
  html: '<p></p>',
  config: {
    swfPath: '/JavaScript/ExtJS/ux/exporter/downloadify.swf',
    downloadImage: '/JavaScript/ExtJS/ux/exporter/download.png',
    width: 62,
    height: 22,
    downloadName: 'export',
  },
  ...
  constructor: function (config) {
    ...
    this.on('afterrender', function () {
      // We wait for the combo to be rendered, so we can look up to grab the component containing it
      self.setComponent(config.component, config)
    })
  },
  ...
  setDownloadify: function (config) {
    ...
    Downloadify.create(this.el.down('p').id, {
      filename: function () {
        return (
          self.getDownloadName() +
          '.' +
          Ext.ux.exporter.Exporter.getFormatterByName(self.formatter).extension
        )
      },
      data: function () {
        return Ext.ux.exporter.Exporter.exportAny(
          self.component,
          self.formatter,
          config,
        )
