...
Ext.define('Ext.ux.grid.menu.RangeMenu', {
	...
	*/
	iconCls: {
		gt: 'ux-rangemenu-gt',
		lt: 'ux-rangemenu-lt',
		get: 'ux-rangemenu-get',
		let: 'ux-rangemenu-let',
		eq: 'ux-rangemenu-eq',
	...
	* </pre>
	*/
	menuItems: ['lt', 'gt', 'lte', 'gte', '-', 'eq', 'neq'],

	itemIconCls: {
		'gt': 'ux-rangemenu-gt',
		'lt': 'ux-rangemenu-lt',
		'eq': 'ux-rangemenu-eq',
		'lte': 'ux-rangemenu-let',
		'gte': 'ux-rangemenu-get',
		'neq': 'ux-rangemenu-neq'
	},
	...
	constructor: function (config) {
		...
		for (i = 0, len = me.menuItems.length; i < len; i++) {
			...
			if (item !== '-') {
				...
				cfg = {
					...
					labelSeparator: '',
					emptyText: this.fieldLabels[item],
					labelWidth: 29,
					listeners: {
						scope: me,
		...
		//me.add('-');
		//this.customFilter = Ext.create('Ext.ux.grid.filter.CustomFilter', { text: 'Custom filters', checked: false, filter: this });
		//me.add(this.customFilter);
	...
	*/
	getValue: function () {
		...
		//if (this.customFilter.getValue() != '') {
		//	result = { custom: this.customFilter.getValue() };
		//}
		return result;
	},
	...
	*/
	setValue: function (data) {
		...
		//if (data['custom']) {
		//	this.customFilter.setValue(data['custom'])
		//}
